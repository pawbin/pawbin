...
const { merge } = require('../utility/utils.js');

let models = {};

fs.readdirSync(path.join(__dirname, "..", 'models')).forEach((file) => {
  models[file.replace(/\..+?$/, "").toLowerCase()] = require('../models/' + file);
});
...
function getQuery(keyword, params, req, res){
  switch (keyword) {
    ...
    case "PageUser":
      ...
      break;
    case "PageCreature":
      //TODO: get creature by page codename
      return {
...
//TODO: use utility/populate
module.exports = () => {
  return function(req, res, next) {
    console.log(req.user);
    res.preRender = (file, data) => {
      let content = fs.readFileSync(path.join( file + (file.includes(".html") ? "" : ".html")), 'utf-8'),
